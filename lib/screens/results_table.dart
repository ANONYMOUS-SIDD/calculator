...
class ResultsTable extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(() {
      ...
      final double totalNetPoints = netResults.fold(0.0, (sum, r) => sum + r.netPoints);
      ...
      return Padding(
        ...
        child: Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildGameTallySection(totalNetPoints, pointsPerRupee, playersCount), const SizedBox(height: 20), const SizedBox(height: 10), _buildPlayerBreakdownSection(netResults, pointsPerRupee)]),
        ),
      );
    });
  }
  ...
  Widget _buildGameTallySection(double totalNetPoints, double pointsPerRupee, int playersCount) {
    return Container(
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                _buildStatContainer(label: "Points", value: totalNetPoints.abs().toStringAsFixed(0), color: totalNetPoints >= 0 ? Colors.green.shade600 : Colors.red.shade600),
          ...
          return Column(
            children: [
              ...
              ...netResults.map((result) {
                ...
                return _buildPlayerRow(player: player, netPointChange: netPointChange, totalAmount: totalAmount, primaryColor: primaryColor, secondaryColor: secondaryColor, playerFlex: playerFlex, pointsFlex: pointsFlex, amountFlex: amountFlex);
              }).toList(),
            ],
          );
        },
  ...
  Widget _buildPlayerRow({required player, required double netPointChange, required double totalAmount, required Color primaryColor, required Color secondaryColor, required int playerFlex, required int pointsFlex, required int amountFlex}) {
    final bool isPositive = netPointChange > 0;
          ...
          Expanded(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 10),
                Expanded(
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_buildPlayerNameText(player.userName), const SizedBox(height: 5), _buildBasePointsContainer(player.pointsEarned.toStringAsFixed(0))]),
                ),
              ],
            ),
          ),
  ...
  Widget _buildBasePointsContainer(String points) {
  ...
  Widget _playerAvatar(player) {
    return Container(
      ...
      child: ClipOval(
        child: player.userImage != null && player.userImage!.isNotEmpty ? Image.network(player.userImage!, fit: BoxFit.cover, width: 36, height: 36, errorBuilder: (context, error, stackTrace) => _defaultIcon()) : _defaultIcon(size: 18),
      ),
    );
  }

  Widget _defaultIcon({double size = 22}) {
    return Icon(Icons.person_rounded, size: size, color: Colors.blue.shade500);
  }
